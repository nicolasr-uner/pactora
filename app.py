...
from utils.drive_manager import search_documents, fetch_document
    ...
    * {{
    ...
    with tabs[1]:
        ...
        with col1:
            ...
            st.write("📁 **Opción 1: Subir desde tu computador**")
            uploaded_file = st.file_uploader("Subir Documento (.docx)", type=["docx"])
            
            st.divider()
            
            # --- Option B: Load from Google Drive ---
            st.write("☁️ **Opción 2: Buscar en Google Drive**")
            drive_search_query = st.text_input("Buscar por nombre de archivo en Drive", placeholder="Ej: PPA-Granja-Solar-2024")
            
            drive_file_bytes = None
            drive_file_name = None
            
            if st.button("🔍 Buscar en Drive"):
                with st.spinner("Buscando en Google Drive..."):
                    drive_results = search_documents(query=drive_search_query)
                    if drive_results:
                        st.session_state['drive_search_results'] = drive_results
                        st.success(f"✅ Se encontraron {len(drive_results)} documento(s).")
                    else:
                        st.session_state['drive_search_results'] = []
                        st.warning("No se encontraron documentos .docx. Verifica que la cuenta de Google está autenticada y que los archivos existen en Drive.")
            
            if st.session_state.get('drive_search_results'):
                drive_results = st.session_state['drive_search_results']
                options_map = {f"{f['name']} ({f.get('modifiedTime','')[:10]})": f['id'] for f in drive_results}
                selected_label = st.selectbox("Seleccionar documento de Drive", list(options_map.keys()))
                selected_id = options_map[selected_label]
                
                if st.button("⬇️ Cargar desde Drive"):
                    with st.spinner("Descargando documento de Drive..."):
                        drive_file_bytes = fetch_document(selected_id)
                        if drive_file_bytes:
                            drive_file_name = selected_label.split(" (")[0] + ".docx"
                            st.session_state['drive_file_bytes'] = drive_file_bytes
                            st.session_state['drive_file_name'] = drive_file_name
                            st.success(f"📄 Documento cargado desde Drive: **{drive_file_name}**")
                        else:
                            st.error("No se pudo descargar el archivo. Verifica los permisos de Drive.")
            
            # Resolve which file to use (local upload takes priority)
            active_file_bytes = None
            active_file_name = None
            if uploaded_file is not None:
                active_file_bytes = uploaded_file.getvalue()
                active_file_name = uploaded_file.name
            elif st.session_state.get('drive_file_bytes'):
                active_file_bytes = st.session_state['drive_file_bytes']
                active_file_name = st.session_state['drive_file_name']
            
            if active_file_bytes:
                st.session_state['current_file_bytes'] = active_file_bytes
                st.session_state['current_file_name'] = active_file_name
                if uploaded_file is not None:
                    st.info(f"📄 Archivo cargado: **{active_file_name}**")
                st.download_button(
                    label="⬇️ Descargar Archivo Original",
                    data=active_file_bytes,
                    file_name=active_file_name,
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                )
            
            if st.button("Procesar y Analizar Riesgo") and active_file_bytes is not None:
                with st.spinner('Extrayendo texto y analizando con Gemini 1.5 Pro...'):
                    # 1. Extract text from active source (local or Drive)
                    text_content = extract_text_from_docx(active_file_bytes)
